...
echo "62.    node v7.10.1 (Installeer deze versie voor de PI-strip!)"
...
echo "101.   - sudo npm install -g pm2"
echo "102.   - sudo npm install express pubnub johnny-five raspi-io --save"
echo ${WHITE}
...
case $keuze in
     ...
     102)
        ...
        if node -v &>/dev/null; then
          cd /home/pi
          echo ${MAGENTA}Open een nieuw terminalvenster.${WHITE}
          echo ${MAGENTA}Voer volgend commando uit: npm install${WHITE}
          echo ${MAGENTA}Installatie voltooid.${WHITE}
        else
          echo ${RED}Installeer eerst node.js${WHITE}
        fi
